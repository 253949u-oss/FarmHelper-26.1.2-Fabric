...
public class WheatFarmingMacro implements IFeature {
    ...
    public void tick() {
        ...
        // Scan for mature wheat every 10 ticks
        if (tickCounter % 10 == 0) {
            scanForWheat();
        }

        // Path and interact with target
        if (targetBlock != null && harvestCooldown == 0) {
            interactWithWheat(targetBlock);
            harvestCooldown = HARVEST_COOLDOWN;
            harvestCount++;
        }
    }

    private void scanForWheat() {
        if (mc.player == null || mc.level == null) return;

        BlockPos playerPos = mc.player.blockPosition();
        double closestDistance = Double.MAX_VALUE;
        BlockPos closestWheat = null;
        ...
        targetBlock = closestWheat;
    }

    private boolean isMatureWheat(BlockPos pos) {
        if (mc.level == null) return false;
        var blockState = mc.level.getBlockState(pos);
        if (blockState.getBlock() != Blocks.WHEAT) return false;
        ...
        if (ageProperty.isPresent()) {
            ...
            return age.equals(7); // Mature wheat
        }
        return false;
    }

    private void interactWithWheat(BlockPos pos) {
        if (mc.player == null || mc.gameMode == null) return;
        // Destroy the wheat block
